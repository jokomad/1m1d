...
async function fetchPairsWithFundingRates() {
    try {
        ...
        // Filter USDT pairs with ABSOLUTE funding rate > 0.00005000
        // We want: 
        // 1. Positive Funding (> 0.00005) -> Potential Longs (if Price > SMA)
        // 2. Negative Funding (< -0.00005) -> Potential Shorts (if Price < SMA)
        const usdtPairs = fundingData
            ...
            .filter(item => Math.abs(item.fundingRate) > 0.00005000)
            .sort((a, b) => Math.abs(b.fundingRate) - Math.abs(a.fundingRate));
        console.log(`Found ${usdtPairs.length} USDT pairs with absolute funding rate > 0.00005000`);
        ...
        // Filter pairs: 
        // 1. Positive Funding AND Price > SMA (Existing Long Strategy)
        // 2. Negative Funding AND Price < SMA (New Short Strategy)
        const relevantPairs = successfulPairs.filter(p => {
            if (p.fundingRate > 0 && p.percentAboveMiddle > 0) return true;
            if (p.fundingRate < 0 && p.percentAboveMiddle < 0) return true;
            return false;
        });
        // Sort by percentAboveMiddle from highest to lowest        
        // Sort by magnitude of deviance from SMA
        relevantPairs.sort((a, b) => Math.abs(b.percentAboveMiddle) - Math.abs(a.percentAboveMiddle));
        console.log(`Successfully processed ${successfulPairs.length}/${usdtPairs.length} pairs`);
        console.log(`Relevant pairs (Long similar/Short similar): ${relevantPairs.length}/${successfulPairs.length}`);
...
async function runScanAndBroadcast() {
    ...
    try {
        ...
        // Filter pairs for Web UI (preserve existing "Long" strategy only: Positive Funding & > SMA)
        // This ensures the webpage remains unchanged
        const webPairs = pairs.filter(p => p.fundingRate > 0 && p.percentAboveMiddle > 0);
        const scanResult = {
            timestamp: new Date().toISOString(),
            scanTime: new Date().toLocaleTimeString(),
            totalPairs: webPairs.length,
            pairs: webPairs
        };
        // Broadcast to all connected WebSocket clients
        broadcastScanResult(scanResult);

        // Check for pairs above 5% SMA (Long Candidates)
        const longAlertPairs = pairs.filter(p => p.fundingRate > 0 && p.percentAboveMiddle > 5);

        // Check for pairs below 5% SMA with Negative Funding (Short Candidates)
        const shortAlertPairs = pairs.filter(p => p.fundingRate < 0 && p.percentAboveMiddle < -5);
        let telegramMessage = '';
        const time = new Date().toLocaleTimeString('en-GB', { hour: '2-digit', minute: '2-digit' });
        if (longAlertPairs.length > 0) {
            longAlertPairs.forEach(p => {
                telegramMessage += `🟢 ${time} - ${p.symbol} +${p.percentAboveMiddle.toFixed(2)}% - ${p.currentPrice} USDT\n`;
            });
        }

        if (shortAlertPairs.length > 0) {
            shortAlertPairs.forEach(p => {
                // Using Red Circle for Short alerts
                telegramMessage += `🔴 ${time} - ${p.symbol} ${p.percentAboveMiddle.toFixed(2)}% - ${p.currentPrice} USDT\n`;
            });
        }
        if (telegramMessage) {
        ...
        console.log(`Found ${pairs.length} relevant pairs`);
        if (longAlertPairs.length > 0) {
            console.log(`⚠️  SENT TELEGRAM ALERT FOR ${longAlertPairs.length} LONG PAIRS (>5%)`);
        }
        if (shortAlertPairs.length > 0) {
            console.log(`⚠️  SENT TELEGRAM ALERT FOR ${shortAlertPairs.length} SHORT PAIRS (<-5%)`);
        }
